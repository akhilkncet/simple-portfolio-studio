...
const AchievementCard = memo(({ achievement, onClick }: { achievement: typeof achievements[0]; onClick: () => void }) => (
  <div
    onClick={onClick}
    className={`bg-[#1a1a1a] border-4 ${borderColor[achievement.color] || 'border-neo-green'} shadow-hard p-2.5 sm:p-4 md:p-5 2xl:p-6 hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all cursor-pointer group h-full flex flex-col`}
    role="button"
    tabIndex={0}
  ...
  >
    ...
    <div className="space-y-1 sm:space-y-1.5 md:space-y-2 flex-grow">
      <span className={`inline-block ${bgColorWithOpacity[achievement.color] || 'bg-neo-green/20'} text-white border ${borderColor[achievement.color] || 'border-neo-green'} px-1.5 sm:px-2 md:px-3 py-0.5 sm:py-1 text-[9px] sm:text-[10px] md:text-xs 2xl:text-sm font-mono font-bold uppercase`}>
        {achievement.category}
      </span>
      <h3 className="font-black text-xs sm:text-sm md:text-base lg:text-lg 2xl:text-xl uppercase leading-tight text-white">{achievement.title}</h3>
      <p className="font-mono text-[10px] sm:text-xs md:text-sm 2xl:text-base text-gray-200">{achievement.organization}</p>
      <p className="font-mono text-[9px] sm:text-[10px] md:text-xs 2xl:text-sm text-gray-300">{achievement.date}</p>
    </div>
  </div>
));
...
export const Achievements = memo(function Achievements() {
  ...
  return (
    <section id="achievements" className="py-12 sm:py-16 md:py-20 2xl:py-28 bg-[#0a0a0a] border-b-4 border-[#333] relative overflow-hidden" aria-label="Achievements & Certifications">
      <div
      ...
      <div className="max-w-[1400px] 2xl:max-w-8xl 3xl:max-w-9xl mx-auto px-4 2xl:px-8 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 sm:mb-10 2xl:mb-14 border-b-4 border-cyan-400 pb-2.5 sm:pb-3.5">
          <h2 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl 2xl:text-7xl font-black uppercase text-cyan-100 tracking-tighter mb-2 md:mb-0">
          ...
          <div className="flex items-center gap-2 mb-2 md:mb-4">
            ...
            <p className="font-mono text-neo-green text-xs sm:text-sm 2xl:text-base font-bold">/// CERTIFICATES_LOADED</p>
          </div>
        </div>

        {/* Tab Navigation */}
        <nav className="flex flex-wrap gap-2.5 sm:gap-3 md:gap-4 mb-6 sm:mb-8 md:mb-10 2xl:mb-14" role="tablist" aria-label="Achievement categories">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              role="tab"
              ...
              className={`flex items-center gap-2 px-3 py-2.5 sm:px-4 sm:py-3 md:px-6 md:py-4 2xl:px-8 2xl:py-5 border-4 shadow-hard font-black uppercase text-xs sm:text-sm md:text-base 2xl:text-lg transition-all duration-200 ${
                activeTab === tab.id
            ...
            >
              <span className="text-base sm:text-lg md:text-xl 2xl:text-2xl" aria-hidden="true">{tab.icon}</span>
              <span className="hidden sm:inline">{tab.label}</span>
              <span className="sm:hidden">{tab.label.slice(0, -1)}</span>
              <span className="ml-1 sm:ml-2 px-1.5 py-0.5 sm:px-2 sm:py-1 border-2 border-white bg-[#0a0a0a] text-white text-xs 2xl:text-sm font-mono">
                {tab.count}
              </span>
            </button>
          ))}
        ...
        {/* Grid Display */}
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4 gap-3 sm:gap-4 md:gap-5 lg:gap-6 2xl:gap-8 mb-6" role="tabpanel">
        ...
        {activeItems.length > 4 && (
          <div className="flex justify-center mb-8 sm:mb-10">
            <button
              ...
              className="px-6 py-3 sm:px-8 sm:py-4 2xl:px-10 2xl:py-5 bg-neo-blue border-4 border-white shadow-hard font-black uppercase text-sm sm:text-base 2xl:text-lg hover:translate-x-2 hover:translate-y-2 hover:shadow-none transition-all"
            >
        ...
        <div className="border-t-4 border-cyan-400 mt-8 sm:mt-12 2xl:mt-16 pt-3 sm:pt-4 flex flex-col sm:flex-row justify-between gap-2 font-mono text-xs 2xl:text-sm text-cyan-500">
...
const CertificateModal = memo(({ certificate, onClose }: { certificate: typeof achievements[0]; onClose: () => void }) => {
  ...
  return (
    ...
    >
      <div
        className="relative max-w-7xl 2xl:max-w-8xl w-full bg-[#1a1a1a] border-4 border-cyan-400 shadow-hard p-4 sm:p-6 2xl:p-8 my-auto"
      ...
      >
        ...
        <div className="space-y-4 sm:space-y-6">
          <div className="border-b-2 border-cyan-400 pb-3 sm:pb-4 pr-12">
            <h3 className="text-xl sm:text-2xl md:text-3xl 2xl:text-4xl font-black uppercase text-cyan-100 mb-2">{certificate.title}</h3>
            <div className="flex flex-wrap gap-2 sm:gap-4 font-mono text-xs sm:text-sm 2xl:text-base">
          ...
          {hasDescription ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 2xl:gap-8">
              ...
              <div className="bg-[#0a0a0a] border-2 border-gray-700 p-4 sm:p-6 2xl:p-8">
                <h4 className="text-lg sm:text-xl 2xl:text-2xl font-black uppercase text-neo-green mb-3 sm:mb-4">/// DETAILS</h4>
                <ul className="font-mono text-sm sm:text-base 2xl:text-lg text-gray-300 leading-relaxed space-y-2 sm:space-y-3 list-none">
...
const AllCertificatesModal = memo(({ items, tabs, activeTab, onClose, onCardClick }: AllCertificatesModalProps) => {
  ...
  return (
    ...
    >
      <div
        className="relative max-w-7xl 2xl:max-w-8xl w-full bg-[#0a0a0a] border-4 border-neo-blue shadow-hard p-4 sm:p-6 md:p-8 2xl:p-10 my-auto"
        onClick={e => e.stopPropagation()}
      >
        <div className="sticky top-0 bg-[#0a0a0a] pb-4 mb-4 border-b-2 border-neo-blue z-10">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-xl sm:text-2xl md:text-3xl 2xl:text-4xl font-black uppercase text-white mb-2">
                {activeTabInfo?.icon} ALL {activeTabInfo?.label}
              </h3>
              <p className="font-mono text-xs sm:text-sm 2xl:text-base text-gray-400">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 md:gap-5 2xl:gap-7">
